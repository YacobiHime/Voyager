...
ライブではstarving対象が0人だった。hungryへ広げた非実行shadowでは、親子関係にある
市民25（`loyal_builder`）から市民11への評価を実データで生成できた。食料移転、移動、関係更新は
行っていない。
...
3. 現在の比較は判断シミュレーションであり、実移動成功、空腹回復、関係変化の外的妥当性は別途測る。
