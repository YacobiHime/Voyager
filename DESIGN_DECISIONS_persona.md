...
### 2026-08-01 Phase 3.6 実装判断 — 詳細だが検証可能な認知profile

P1を遺伝する正本として維持し、その7数値特性から価値、動機、欲求優先度、規範、対処、
感情力学、意思決定傾向を導出する`voyager-cognition-v1`を追加した。状況ごとには、本人・相手の困窮、
有向関係、対人記憶、距離、資源から5つのactive goalを再計算する。全ての導出値は入力・重み・寄与を
traceとして保持し、同じP1と状況から同じ結果を再現できる。

この方式を選んだ理由は、詳細さを増やしつつ、既存35市民を根拠のない新しい遺伝値で埋めず、
旧appraisalとの比較条件を維持するためである。したがって、詳細次元は独立した心理因子ではなく、
7特性から作る操作的な射影である。名称を人間の心理尺度と同一視しない。

v2の初回scoreは正の構成概念数が増えただけで援助に偏ったため、各bounded signalを中立点0.5からの
正負の証拠へ変換した。修正後の決定論的比較では222場面中16場面で判断が変わった。
係数は`experimental-unvalidated`としてconfigへ分離し、ライブ援助の既定は観測済みv1を維持する。
詳細定義、分布、再現手順、限界は`COGNITIVE_MODEL.md`を正本とする。
